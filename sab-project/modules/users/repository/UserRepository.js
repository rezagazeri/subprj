...
class UserRepository extends BaseRepository {
  ...
  async list (options) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        findParams.include = [{
          model: options.ctx.getUserGenderRepository().repo,
          as: "gender",
        },{
          model: options.ctx.getUserSituationRepository().repo,
          as: "situations",
        },{
          model: options.ctx.getUserRoleRepository().repo,
          as: "userRoles",
          include: [{
            model: options.ctx.getOilChartStructureRepository().repo,
            as: "oilChartStructure",
          },{
            model: options.ctx.getRoleListRepository().repo,
            as: "roleList",
          }]
        }];
  ...
  async register (options) {
    return new Promise(async (resolve, reject) => {
      try {
        options.registerDate = new Date()
        options.situationId = constants.user.situation.pending
