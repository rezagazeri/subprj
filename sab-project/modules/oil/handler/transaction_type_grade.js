...
async function handleGetAll(ctx, req, res) {
    ...
    let items = await ctx.getTransactionTypeGradeRepository().selectList(options, pagination)
...
async function handleGetById(ctx, req, res) {
    ...
    let result = await ctx.getTransactionTypeGradeRepository().selectById(id)
