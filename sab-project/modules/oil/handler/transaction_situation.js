...
async function handleGetAll(ctx, req, res) {
    ...
    let items = await ctx.getTransactionSituationRepository().selectList(options, pagination)
...
async function handleGetById(ctx, req, res) {
    ...
    let result = await ctx.getTransactionSituationRepository().selectById(id)
